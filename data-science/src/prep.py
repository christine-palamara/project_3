...
def main(args):  # Write the function name for the main data preparation logic
    ...
    mlflow.log_metric('train_size', train_df.shape[0])  # Log the train dataset size with number of rows
    mlflow.log_metric('test_size', test_df.shape[0])  # Log the test dataset size with number of rows
...
if __name__ == "__main__":
    ...
    # Parse Arguments
    args = argparse.ArgumentParser()  # Call the function to parse arguments
    ...
    lines = [
        ...
        f"Test-train ratio: {args.test_train_data}",  # Print the test_train_ratio
    ]
